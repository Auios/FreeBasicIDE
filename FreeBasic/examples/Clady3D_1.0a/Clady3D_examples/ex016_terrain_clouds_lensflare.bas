...
Dim as nTexture texclouds=nLoadTexture("media/clouds/cloud4.png")
Dim as nClouds cloud=nCreateClouds(texclouds,3,1,300)
nSetEntityFlag(cloud,MF_FOG,1)
nSetEntityPosition(cloud,2500,1500,2500)

Dim as nTexture texlens=nLoadTexture("media/lensflare/flares.jpg")
Dim as nLensFlare flare=nCreateLensFlareNode(texlens)
nSetLensFlareScale(flare,0.3,0.3)
nSetEntityPosition(flare,0,4000,-1000)
...
While(EngineRun)'Returns 1 if the engine is running.
    ...
    nSetEntityLocalRotation(cloud,0,0.05,0)
