...
function hConnect6 overload (ss as SOCKET, ip as ulongint, port as integer ) as integer
    dim sa as sockaddr_in
...
function hConnect6 overload (ss as SOCKET, ip as ulongint, port as long, byref LocalIP as integer,byref LocalPort as integer) as integer
    dim sa as sockaddr_in
...
function hBind6( byval ss as SOCKET, byval port as integer, iIP as ulongint = INADDR_ANY ) as integer
    dim sa as sockaddr_in
...
function hSendUDP6( ss as SOCKET, IP as ulongint, Port as long, buffer as zstring ptr, bytes as long ) as integer  
    dim sa as sockaddr_in
    sa.sin_port         = htons(port)
    sa.sin_family       = AF_INET
    sa.sin_addr.S_addr  = IP
