...
sub RotateScaleHQ( dst as FB.IMAGE ptr = 0, src as FB.IMAGE ptr, positx as integer, posity as integer, angle as single, SCALE as single)
  ...
  asm   'Draw Pixel Macros
    ...
    sub edx,[SUBY]                '| |RY=cMULNUMB-SUBY
    mov [RX],eax                  '| /
    mov [RY],edx                  '/  
    
    movzx eax, PixPtr [_SRCPTR_+0]  '\ C4=-512:C1=SRCPIX(0)
    mov [C1],al                     '/
    ...
    movzx edx, PixPtr [_SRCPTR_+1]  '\ C2 = SRCPIX(1)
    mov [C2],dl                     '/
    ...
    movzx eax,PixPtr[_SRCPTR_+128] '\ C3 = SRCPIX(128)
    mov [C3],al                    '/
    ...
    movzx edx,PixPtr[_SRCPTR_+129]'\ C4 = SRCPIX(129)
    mov [C4],dl                   '/
