...
dim shared as integer C1,C2,C3,C4
dim shared as integer RX,SUBX,RY,SUBY    
...
sub RotateScaleHQ( dst as FB.IMAGE ptr = 0, src as FB.IMAGE ptr, positx as integer, posity as integer, angle as single, SCALE as single)
  ...
  asm   'Draw Pixel Macros
    ...
    mov eax,cLowMul               '  \
    mov edx,cLowMul               '\ |
    sub ax,[SUBX]                 '| | RX = cMULNUMB-SUBX
    sub dx,[SUBY]                 '| |RY=cMULNUMB-SUBY
    mov [RX],ax                   '| /
    mov [RY],dx                   '/  
    'xor edx,edx                     '\
    mov eax, PixPtr [_SRCPTR_+0]    '| C4=-512:C1=SRCPIX(0)
    mov [C1],eax                    '/
    'cmp dword ptr [SRX],127       '\ if MX=127 then
    'jne _HQR8_ELSE_IF_MX_         '/
    'movzx eax, PixPtr [_DSTPTR_]    '\
    'mov [C2],al                     '| C2=DSTPIX:C4=C2 
    'mov [C4],al                     '|
    'inc edx                         '|
    'jmp _HQR8_END_IF_MX_            '/
    '_HQR8_ELSE_IF_MX_:            '> else
    mov eax, PixPtr [_SRCPTR_+4]    '\ C2 = SRCPIX(1)
    mov [C2],eax                    '/
    '_HQR8_END_IF_MX_:             '> end if
    'cmp dword ptr [SRY],127         '\ if MY=127 then 
    ...
    mov eax,PixPtr[_SRCPTR_+512]  '\ C3 = SRCPIX(128)
    mov [C3],eax                  '/
    '_HQR8_END_IF_MY_:               '> end if
    'or edx,edx                    '\ if C4=-512 then
    'jnz _HQR8_SKIP_IF_C4_         '/
    #if 0
    'mov eax,[SRY]                   '\
    ...
    mov eax,PixPtr[_SRCPTR_+516]   '\ C4 = SRCPIX(129)
    mov [C4],eax                   '/
    ...
    movd mm4,[RX] 'RX:SUBX    
    punpcklwd mm3,mm3
    pshufw mm4,mm4,&b10011100
    movq mm5,[C1] 'C1:C2:C3:C4    
    pmulhuw mm3,mm4
    psllq mm5,cLowBits
    psllq mm3,cLowBits
    pmulhuw mm3,mm5    
    'mov eax,255                   '| \
    'cmp ebx,eax                   '  | if COR > 255 then COR=255
    'cmovg ebx,eax                 '  /
    ...
    shr edx,cLowBits              '| |C2=(C2*((SUBX*RY) shr cUSEBITS)) shr cUSEBITS
    imul eax,[C1]                 '| |
    imul edx,[C2]                 '| |
    sar eax,cLowBits              '| |
    adc eax,0                     '| |
    ...
    shr edx,cLowBits              '| |C4=(C4*((SUBX*SUBY) shr cUSEBITS)) shr cUSEBITS 
    imul eax,[C3]                 '| |
    imul edx,[C4]                 '| |
    shr eax,cLowBits              '| |
    adc eax,0                     '| |
  ...
  asm   'Drawing Rotate/Scaled Block
    ...
    mov _DSTPTR_,[DSTPTR]     '/
    'mov eax,cMULNUMB         '\
    'movd MM7,eax             '| cMULNUMB:cMULNUMB
    'punpckldq MM7,MM7        '/
    mov _PY_,[SY]          '\
    movd MM0,esp           '|
    movd MM1,ebp           '|
