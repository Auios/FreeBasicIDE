...
sub RotateScale( dst as FB.IMAGE ptr = 0, src as FB.IMAGE ptr, positx as integer, posity as integer, angle as single, SCALE as single , ColorKey as ulong = &hFFFF00FF )
...
sub RotateScaleHQ_SP( dst as FB.IMAGE ptr = 0, src as FB.IMAGE ptr, positx as integer, posity as integer, angle as single, SCALE as single)
  ...
  if SCALE < 1 then
    rotateScaleHQ(dst,src,positx,posity,angle,SCALE)      
    exit sub
  end if
  
  scope 'setup rotation parameters
    dim as single SPDUP = (SCALE/3)^(1/2.5)
    SPDUPX = cint(SPDUP): SPDUPY = cint(SPDUP+.5)    
    if INIT=0 then      
      asm
        inc dword ptr [INIT]
